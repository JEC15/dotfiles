...
# Based on https://git.tjdev.de/thetek/waybar-weather
...
OPENWEATHERMAP_API_KEY_FILE_PATH = os.path.expanduser('~/.owm-key')

try:
    with open(OPENWEATHERMAP_API_KEY_FILE_PATH, 'r') as f:
    ...
    print(f"Error: No API key file found. Create one at {OPENWEATHERMAP_API_KEY_FILE_PATH}")
    sys.exit(1)
except PermissionError:
    print(f"Error: Can't read {OPENWEATHERMAP_API_KEY_FILE_PATH} - check file permissions")
    sys.exit(1)
...
def get_current_weather_data() -> dict:
    ...
    res = make_request(f'data/2.5/weather?lang={LANG}&lat={LATITUDE}&lon={LONGITUDE}&units=metric')
...
def get_forecast_data() -> dict:
    ...
    res = make_request(f'data/2.5/forecast?lang={LANG}&lat={LATITUDE}&lon={LONGITUDE}&units=metric')
...
def waybar_widget(data: dict) -> str:
    ...
    # return f'{colorize(weather, GRAY)} {temperature}°'
...
def waybar_current(data: dict) -> str:
    ...
    # generate output
    output = ''
    output += waybar_entry('clima',  colorize(weather, YELLOW))
    output += waybar_entry('temp',     f'{colorize(f"{temperature} °C", ORANGE)}{colorize(", sensación térmica ", GRAY)}{colorize(f"{temperature_felt} °C", ORANGE)}')
    output += waybar_entry('humedad', colorize(f'{humidity} % HR', RED))
    output += waybar_entry('viento',     f'{colorize(f"{wind_speed} m/s", PURPLE)} {colorize(f"({wind_direction})", GRAY)}')
    output += waybar_entry('lluvia',     colorize(f"{rainfall} mm", BLUE))
    return output
...
def waybar():
    ...
    tooltip = colorize('clima actual', GREEN) + '\n' + current + '\n' + \
              colorize('pronostico', GREEN) + '\n' + forecast
